...
export default function PayButton({ product, variant = 'full' }: { product: PayProduct; variant?: 'full' | 'simple' }) {
  const { token } = useAuth();
  ...
  return (
    <div className="pt-1">
      {variant === 'full' ? (
        ...
        <div className="">
          <button onClick={() => handlePay()} className="w-full rounded-full bg-black px-6 py-3 text-sm font-medium text-white hover:bg-black/90">
            Acheter
          </button>
        </div>
      )}
